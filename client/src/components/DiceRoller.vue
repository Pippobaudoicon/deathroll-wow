    ...
    <div class="space-y-6">
      ...
      <div class="dice-container">
        <div 
          ...
          :class="{
            'rolling': isRolling,
            'animate-shake': lastRoll?.isEliminating
          }"
        >
          <span v-if="displayNumber">
            {{ displayNumber }}
          </span>
      ...
      <!-- Last Roll Info -->
      <div v-if="lastRoll" class="space-y-2">
        <div :class="[
          'p-3 rounded-lg',
...
// Watch for new rolls to show result immediately
watch(lastRoll, (newRoll, oldRoll) => {
  if (newRoll && newRoll.id !== oldRoll?.id) {
    // Show result immediately
    displayNumber.value = newRoll.result
...
const handleRoll = async () => {
  ...
  // Wait 1 second before sending roll request
  await new Promise(resolve => setTimeout(resolve, 500))
  
  // Send roll request - result will come back via socket event
  gameStore.rollDice()
}
</script>
